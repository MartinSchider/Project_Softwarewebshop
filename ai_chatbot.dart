...
class AIChatBot {
  ...
  static Future<String?> _handleGreeting(String q, List<dynamic> _) async {
    if (q.contains('hello') || q.startsWith('hi') || q.startsWith('hey') || q.contains('good day')) {
  ...
  static Future<String?> _handleAvailability(String q, List<dynamic> products) async {
    if (!(q.contains('available') || q.contains('in stock') || q.contains('stock'))) return null;
    for (final p in products) {
      try {
        final name = (p.name as String).toLowerCase();
        if (q.contains(name)) {
  ...
  static Future<String?> _handleCheap(String q, List<dynamic> products) async {
    if (!(q.contains('cheap') || q.contains('cheapest') || q.contains('affordable'))) return null;
    ...
    final list = sorted.take(3).map((p) => '${p.name} (\$${(p.price as num).toStringAsFixed(2)})').join(', ');
    return 'Our cheapest items are: $list';
  }

  static Future<String?> _handleExpensive(String q, List<dynamic> products) async {
  ...
  static Future<String?> _handleProductList(String q, List<dynamic> products) async {
    // Check if the query is asking for products or items
    if (!(q.contains(' show ') || q.contains(' list ') || q.contains(' what ') || 
          q.contains(' items ') || q.contains('products') || q.contains(' all '))) {
      return null;
    }
